#!/bin/bash
set -e
BIT_TAG=$(curl -u ${{ secrets.GAPI_ID }}:${{ secrets.GAPI_TOKEN }} https://api.github.com/repos/chriswalz/bit/releases|grep tag|head -n1|cut -d '"' -f4|sed 's/https:\/\/github.com\/chriswalz\/bit\/releases\/tag\/v//g')
aria2c $(curl -u ${{ secrets.GAPI_ID }}:${{ secrets.GAPI_TOKEN }} https://api.github.com/repos/chriswalz/bit/releases|grep browser_download_url|grep linux_amd64.tar.gz|head -n1|cut -d '"' -f4)
