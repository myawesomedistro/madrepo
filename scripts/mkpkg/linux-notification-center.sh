#!/bin/bash
set -e
DEADD_TAG=$(wget -O- --user '$GITHUB'_API_AUTH https://api.github.com/repos/phuhl/linux_notification_center/releases|grep tag|head -n1|cut -d '"' -f4|sed 's/https:\/\/github.com\/phuhl\/linux_notification_center\/releases\/tag\///g')
...
wget -O linux-notification-center/usr/bin/deadd-notification-center $(wget -O- --user '$GITHUB'_API_AUTH https://api.github.com/repos/phuhl/linux_notification_center/releases|grep browser_download_url|head -n1|cut -d '"' -f4)
