#!/bin/bash
set -e
YOSEMITE_VERSION=`echo $(wget -O- --user rauldipeas:$GITHUB_API_AUTH https://api.github.com/repos/supermarin/YosemiteSanFranciscoFont/commits/master|grep date|head -n 1|cut -d '"' -f4|cut -d \: -f1|cut -d \T -f1)`
