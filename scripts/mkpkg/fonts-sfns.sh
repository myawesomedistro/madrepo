#!/bin/bash
set -e
YOSEMITE_VERSION=`echo $(curl -u $GAPI_AUTH https://api.github.com/repos/supermarin/YosemiteSanFranciscoFont/commits/master|grep date|head -n 1|cut -d '"' -f4|cut -d \: -f1|cut -d \T -f1)`
