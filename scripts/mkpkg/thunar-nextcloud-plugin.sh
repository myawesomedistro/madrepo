...
TNP_VERSION=`echo $(curl -u $GITHUB_API_ID:$GITHUB_API_TOKEN https://api.github.com/repos/frederikmoellers/thunar-nextcloud-plugin/commits/master|grep date|head -n 1|cut -d '"' -f4|cut -d \: -f1|cut -d \T -f1)`
