#!/bin/bash
set -e
GLORIOUS_TAG=$(curl -u $GITHUB_API_ID:$GITHUB_API_TOKEN https://api.github.com/repos/manilarome/lightdm-webkit2-theme-glorious/releases|grep tag|head -n1|cut -d '"' -f4|sed 's/https:\/\/github.com\/manilarome\/lightdm-webkit2-theme-glorious\/releases\/tag\///g')
