#!/bin/bash
set -e
aria2c http://mirrors.kernel.org/ubuntu/pool/universe/x/xfce4-panel-profiles/$(curl -s http://mirrors.kernel.org/ubuntu/pool/universe/x/xfce4-panel-profiles/|grep all.deb|head -n1|cut -d '"' -f2)
dpkg-deb -x xfce4-panel-profiles_1.0.10-0ubuntu1_all.deb xfce4-panel-profiles
