#!/bin/bash
set -e
TELA_TAG=$(curl -u $GITHUB_API_ID:$GITHUB_API_TOKEN https://api.github.com/repos/vinceliuice/Tela-icon-theme/releases|grep tag|head -n1|cut -d '"' -f4|sed 's/https:\/\/github.com\/vinceliuice\/Tela-icon-theme\/releases\/tag\///g')
