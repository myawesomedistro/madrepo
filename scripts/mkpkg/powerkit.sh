...
mkdir -pv powerkit/build
cd powerkit/build
qmake PREFIX=/usr CONFIG+=install_udev_rules ..
make
make INSTALL_ROOT=package install
mkdir -pv powerkit
mv -v app/package/* powerkit/
sed -i 's/XFCE;//g' powerkit/etc/xdg/autostart/powerkit.desktop powerkit/usr/share/applications/powerkit.desktop
mkdir -pv powerkit/DEBIAN
POWERKIT_TAG=$(wget -O- https://api.github.com/repos/rodlie/powerkit/releases|grep tag|head -n1|cut -d \" -f4|sed 's/https:\/\/github.com\/rodlie\/powerkit\/releases\/tag\///g')
