...
apt-fast update
lz4cat -d /var/lib/apt/lists/*deb-multimedia*amd64*Packages.lz4|grep ^Package:|awk '{print $2}'|sort -u|xargs apt-fast download
lz4cat -d /var/lib/apt/lists/*deb-multimedia*i386*Packages.lz4|grep ^Package:|awk '{print $2}'|sort -u|sed 's/$/:i386/g'|xargs apt-fast download