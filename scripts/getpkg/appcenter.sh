...
apt-fast update
grep ^Package: /var/lib/apt/lists/*appcenter*_Package:s|awk '{print $2}'|sort -u|xargs apt-fast download
sudo add-apt-repository ppa:elementary-os/daily
grep ^Package: /var/lib/apt/lists/*elementary-os*_Package:s|awk '{print $2}'|sort -u|xargs apt-fast download