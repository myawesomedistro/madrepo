...
apt-fast update
lz4cat -d /var/lib/apt/lists/*appcenter*_Packages.lz4|grep ^Package:|awk '{print $2}'|sort -u|xargs apt-fast download
sudo add-apt-repository -n ppa:elementary-os/stable
sudo sed -i 's/focal/bionic/g' /etc/apt/sources.list.d/elementary*
sudo add-apt-repository ppa:elementary-os/daily
lz4cat -d /var/lib/apt/lists/*elementary-os*_Packages.lz4|grep ^Package:|awk '{print $2}'|sort -u|xargs apt-fast download