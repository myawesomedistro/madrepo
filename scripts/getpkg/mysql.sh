#!/bin/bash
set -e
aria2c http://mirrors.kernel.org/ubuntu/pool/main/m/mpdecimal/$(wget -O- http://mirrors.kernel.org/ubuntu/pool/main/m/mpdecimal/|grep libmpdec2|grep amd64.deb|cut -d '"' -f2|head -n1|tail -n2)
...
grep ^Package: /var/lib/apt/lists/*mysql*_Package:s|awk '{print $2}'|sort -u|xargs apt-fast download