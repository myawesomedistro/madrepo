#!/bin/bash
set -e
#aria2c $(curl -u ${{ secrets.GAPI_ID }}:${{ secrets.GAPI_TOKEN }} https://api.github.com/repos/VSCodium/vscodium/releases|grep browser_download_url|grep amd64.deb|head -n1|cut -d '"' -f4)
echo 'deb https://paulcarroty.gitlab.io/vscodium-deb-rpm-repo/debs/ vscodium main'|sudo tee /etc/apt/sources.list.d/vscodium.list
wget -O- https://gitlab.com/paulcarroty/vscodium-deb-rpm-repo/-/raw/master/pub.gpg|gpg --dearmor|sudo tee /etc/apt/trusted.gpg.d/vscodium.gpg>/dev/null
apt-fast update
cat /var/lib/apt/lists/*vscodium*_Packages|grep ^Package:|awk '{print $2}'|sort -u|xargs apt-fast download